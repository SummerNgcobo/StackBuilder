...
function sortKeys(input, options = {}) {
...
function jsonStringifyAndEscapeForInlineJS(input, { baseIndent = 0, jsonIndent = 2, prettyPrint = true } = {}) {
  ...
  try {
    if (isPlainObject(input) || Array.isArray(input)) {
      sorted = sortKeys(input, { deep: true });
    }
  } catch (e) {
    console.error("Error sorting Island keys", e);
  }
  return escapeForInlineJS(
    JSON.stringify(sorted ?? input, null, jsonIndent)
  ).replace(/\n/gm, `
${indentString}`);
}
var viteRoot;
...
var serializeIslandDataForInlineScript = (tweakedIslandsArray, perServerRequestData, baseIndent = 0, prettyPrint, isUngatedForClientOnlySupplementalFieldSerialization = false) => {
  const islandsArrayToSerialize = tweakedIslandsArray.map(
    ({
      ...
      if (resolvedDataDependencies) {
        output.resolvedDataDependencies = resolvedDataDependencies;
      }
      if (sharedStateID) {
...
function toRelativePath(config, path17) {
  return path17.replace(`${config.root}/`, "");
}
...
function analyzeSourceCodeUsages(ctx, moduleInfo, config, code) {
  ...
  traverse(sourceAst, {
    ImportDeclaration(path17) {
      if (t.isStringLiteral(path17.node.source, {
        value: "@hubspot/cms-components"
      })) {
        for (const specifier of path17.node.specifiers) {
    ...
    JSXOpeningElement(path17) {
      if (t.isJSXIdentifier(path17.node.name, {
        name: moduleComponentLocalImportName
      })) {
        let cmsModuleName;
        let cmsModulePath;
        let cmsModuleLabel;
        for (const prop of path17.node.attributes) {
...
var promiseForNewWritableStream = () => {
  ...
  const streamDonePromise = new Promise((resolve2, reject) => {
...
// src/plugins/hsStaticAssetPlugin.ts
import { createProxyMiddleware } from "http-proxy-middleware";
function hsStaticAssetPlugin() {
  return {
    name: "hsstatic-assets-plugin",
    enforce: "pre",
    configureServer(server) {
      return () => {
        const getDomainInfo = (req) => {
          const { authedPortalId, authedHublet, sslEnabled, authedEnv } = req.app.locals;
          const env = authedEnv === "qa" ? "qa" : "";
          const previewDomain = `${authedPortalId}.hubspotpreview${env}-${authedHublet}.com`;
          const protocol = sslEnabled ? "https" : "http";
          return { previewDomain, protocol };
        };
        server.middlewares.use(
          createProxyMiddleware(
            (__pathname, req) => {
              var _a;
              if (!((_a = req.url) == null ? void 0 : _a.match("^/hs/hsstatic/.*")))
                return false;
              const { authedPortalId, authedHublet, authedEnv } = req.app.locals;
              return !!(authedPortalId && authedHublet && authedEnv);
            },
            {
              changeOrigin: true,
              secure: false,
              xfwd: true,
              logLevel: "silent",
              // This is a placeholder target that gets overridden by the router
              // but is a required parameter for initialization
              target: "https://hubspot.com",
              router: (req) => {
                const { previewDomain, protocol } = getDomainInfo(req);
                return `${protocol}://${previewDomain}`;
              },
              onProxyReq: (proxyReq, req, __res) => {
                const { previewDomain, protocol } = getDomainInfo(req);
                proxyReq.setHeader("X-Forwarded-Proto", protocol);
                proxyReq.setHeader("host", previewDomain);
                proxyReq.setHeader(
                  "referer",
                  `${protocol}://${previewDomain}/content-rendering/v1/public/_hcms/preview/template/multi`
                );
              }
            }
          )
...
function getPackageJson(packagePath2) {
  return JSON.parse(readFileSync2(join2(packagePath2, "package.json")).toString());
}
...
`);
  ...
  const reactAndIslandsPlugins = isProjectWithReact || process.env.VITEST ? [
    react({
      babel: {
        plugins: [
          require2.resolve("babel-plugin-styled-components"),
          require2.resolve("styled-jsx/babel"),
...
function trackUsage(action) {
  const accountConfig = trackingAccountId && getAccountConfig(trackingAccountId);
...
var setTrackingInfo = (accountId, env) => {
  ...
  while (offsetSoFar < body.length && offsetSoFar !== -1) {
    ...
    if (startOffset >= 0) {
      ...
      if (pathEndOffset >= 0 && pathEndOffset < nextNewlineOffset) {
        const path17 = body.substring(
          startOffset + localJsRenderStartString.length,
          pathEndOffset
        );
        offsetSoFar = pathEndOffset + 1;
        if (startOffset >= 0 && path17) {
...
var getHSProjectConfig = (hsProjectConfigPath) => {
  try {
    return JSON.parse(fs2.readFileSync(hsProjectConfigPath, "utf8"));
  } catch (e) {
    console.warn(
...
var renderLocalError_default = (path17, error) => {
  const stackWithColorClasses = Converter.toHtml(error.stack);
  return `<div data-js-rendered="error" style="${darkErrorStyles}">Error rendering ${path17}<br/><br/><code><pre>${stackWithColorClasses}</pre></code></div>`;
};
...
function initiateSession(accountId, resetSession = false) {
...
async function initialPreviewBufferUpload(accountId, src, dest, filePaths, uploadOptions) {
  ...
  const results = await uploadFolder(
    ...
    rest,
    filePaths.map(
      (filePath) => filePath.replaceAll(path3.sep, path3.posix.sep)
    )
  );
...
function buildUploadFileToPreviewBufferCallback(accountId, src, dest) {
  return (filePath) => {
    ...
    if (path3.dirname(filePath) === src && ["theme.json", "fields.json"].includes(path3.basename(filePath))) {
...
function getDesignManagerPath(src, dest, file) {
  ...
  const regex = new RegExp(`^${escapeRegExp(normalizedSrc)}`);
  const relativePath = normalizedFile.replace(regex, "");
  return convertToUnixPath(path3.join(dest, relativePath));
}
...
function isModuleFolderChild(filePath) {
  ...
  if (pathParts.length <= 1)
    return false;
  return pathParts.slice(0, pathParts.length - 1).some((part) => getExt(path3.normalize(part)) === MODULE_EXTENSION);
}
...
async function initiatePreviewSession(authedPortalId, absoluteWebRoot, themePreviewOptions) {
  let res;
  try {
    res = (await initiateSession(authedPortalId, themePreviewOptions.resetSession)).data;
  } catch (err) {
  ...
  if (token) {
    const { dest, filePaths, startProgressBar } = themePreviewOptions;
    const absoluteDest = `@preview/${token}/${dest}`;
    const nonJSRFilePaths = filePaths.filter(
      (filePath) => !filePath.startsWith(path4.join(absoluteWebRoot, "components"))
    );
    if (tokenStatus === "NEW") {
      ...
      const filesToUpload = [...changedFiles, ...addedFiles].map((filePath) => {
        const relPath = path4.relative(absoluteDest, filePath.substring(1));
        return path4.join(absoluteWebRoot, relPath);
      });
...
var renderComponentForPlaceholder = (indexForThisRender, localRenderPlaceholder, webRoot, viteServer, pageUrl, basePath, environmentConfig, hsProjectJsonProjectName, webRootProjectComponent) => {
  ...
  const resolvedPath = normalizePath3(
    path5.resolve(webRoot, partialPath, componentFileName)
  );
...
async function proxySpecificPage(request2, response, portalId, hublet, contentId, webRoot, viteServer, projectName, projectComponentPath, jsRenderingGates, urlToProxy, fullOriginalUrl) {
  let accountId;
  let proxyEnv;
...
async function infillReactModulesAndWriteResponse(response, pageHtml, webRoot, fullOriginalUrl, environmentConfig, viteServer, projectName, projectComponentPath, jsRenderingGates, prettyPrint) {
...
async function signUrl(authedPortalId, unsignedUrl, requestBody) {
  const formData = {
    module: [JSON.stringify(requestBody)]
  };
  const request2 = {
    unsignedUrl,
    httpMethod: "POST",
  ...
  try {
    const response = await hsCliHttp7.post(authedPortalId, {
      url: "/url-signer-rest/v1/signing/for-form",
      headers: {
        "Content-Type": "application/json",
        Accept: "application/json"
      },
...
async function renderLocalProjectModulePreview(moduleName, authedPortalId, authedEnv, authedHublet, webRoot, viteServer, jsRenderingGates, projectName, projectComponentPath, fullOriginalUrl, cookieValue, fieldOverrides = {}) {
  const modulePath = normalizePath4(
    path6.resolve(webRoot, "components/modules/", moduleName)
  );
  ...
  const currentHubApiDomainOverride = process.env.HUBAPI_DOMAIN_OVERRIDE;
  process.env.HUBAPI_DOMAIN_OVERRIDE = `api.hubspot`;
  const previewDomain = `https://${authedPortalId}.hubspotpreview${authedEnv === "qa" ? "qa" : ""}-${authedHublet}.com`;
  const templatePreviewPath = `_hcms/preview/template/multiform`;
  const unsignedUrl = `${previewDomain}/${templatePreviewPath}?${queryParams}`;
  const signedUrl = await signUrl(authedPortalId, unsignedUrl, localModule);
  const postBody = `module=${encodeURIComponent(JSON.stringify(localModule))}`;
  let templatePreview;
  try {
    const authHeaders = {};
    const accountConfig = getAccountConfig2(authedPortalId);
    if (accountConfig && accountConfig.authType === "personalaccesskey") {
      const accessToken = await accessTokenForPersonalAccessKey(authedPortalId);
    ...
    const response = await fetch(signedUrl, {
      ...
      headers: {
        "content-type": "application/x-www-form-urlencoded",
        Accept: "text/html",
        referer: `https://api.hubspot${authedEnv === "qa" ? "qa" : ""}.com`,
        cookie: cookieValue,
        "User-Agent": "HubSpot CMS Dev Server",
        ...authHeaders
      },
    ...
    templatePreview = await response.text();
  } catch (error) {
...
var partialRouteHandler = async (request2, response) => {
  ...
  const partialPath = normalizePath5(
    path7.resolve(webRoot, "components/partials/", partialName)
  );
      ...
      for (const localProjectModule of localProjectModules) {
        ...
        localProjectModulePreviewPromises.push(
          renderLocalProjectModulePreview(
            path7.basename(localProjectModule.path),
            authedPortalId,
            authedEnv,
            authedHublet,
            webRoot,
...
function PartialsList({ webRoot, partialNames }) {
  if (partialNames.length === 0) {
    return /* @__PURE__ */ jsx2(EmptyPartialsList, { webRoot });
  }
...
function ModulesList({ webRoot, moduleNames }) {
  if (moduleNames.length === 0) {
    return /* @__PURE__ */ jsx4(EmptyModulesList, { webRoot });
  }
...
var linksToAllModuleRouteHandler = async (request2, response) => {
  ...
  const { html: responseHtml } = await renderOnServer(
    /* @__PURE__ */ jsx5(ModulesList_default, { webRoot, moduleNames })
  );
  response.status(200).set({ "Content-Type": "text/html" }).end(responseHtml);
};
...
var moduleRouteHandler = async (request2, response) => {
  ...
  const modulePath = normalizePath6(
    path8.resolve(webRoot, "components/modules/", moduleName)
  );
...
var moduleFieldsJsonRouteHandler = async (request2, response) => {
  try {
    ...
    const { webRoot, viteServer } = request2.app.locals;
    const { moduleName } = request2.params;
    const modulePath = normalizePath7(
      path9.resolve(webRoot, "components/modules/", moduleName)
    );
...
async function fetchModuleByPath(accountId, path17) {
  return (await hsCliHttp8.get(accountId, {
    url: `${DESIGN_MANAGER_API_PATH}/modules/by-path/${path17}?portalId=${accountId}`
  })).data;
}
async function fetchTemplateByPath(accountId, path17) {
  return (await hsCliHttp8.get(accountId, {
    url: `${DESIGN_MANAGER_API_PATH}/templates/by-path/${path17}?portalId=${accountId}`
  })).data;
}
async function fetchModulePageDependencies(accountId, path17) {
  const url3 = `${DESIGN_MANAGER_API_PATH}/modules/list-pages?modulePath=${path17}`;
...
var legacyRouteHandler = async (request2, response, next) => {
  ...
  const { html: responseHtml } = await renderOnServer(
    /* @__PURE__ */ jsxs4(Fragment, { children: [
      ...
      /* @__PURE__ */ jsx8(PartialsList, { webRoot, partialNames }),
      /* @__PURE__ */ jsx8("h2", { children: "React modules" }),
      /* @__PURE__ */ jsx8(ModulesList_default, { webRoot, moduleNames }),
...
function proxyPathPageResourceRedirect(req, res, next) {
  ...
  fetch2(urlToProxy).then((response) => response.body.pipe(res)).catch((err) => console.log(err));
}
...
async function modulePreviewRouteHandler(request2, response) {
  ...
  const modulePath = normalizePath8(
    path10.resolve(webRoot, "components/modules/", moduleName)
  );
...
var hublModuleRouteHandler = async (request2, response) => {
  ...
  const calculatedPath = `@preview/${localPreviewToken}/@projects/${modulePath}.module`;
...
var HUBSPOT_PREVIEW_BASE_TAG_REGEX = /<base\s+[^>]*href=["'][^"']*(?:hubspotpreview(?:qa)?)[^"']*["'][^>]*>/i;
var templateRouteHandler = async (request2, response) => {
  ...
  try {
    ...
    const finalHtml = pageHtml.replace(
      HUBSPOT_PREVIEW_BASE_TAG_REGEX,
      '<base href="/" />'
    );
...
async function handleGetModulePages(req, res, next) {
  const { path: path17, portalId } = req.query;
  if (!path17 || !portalId) {
...
function handleOpenFile(req, res, next) {
  ...
  try {
    ...
    spawn(command, [path17], { detached: true }).unref();
    res.sendStatus(200);
  } catch (err) {
    next(err);
  }
  ...
  return Promise.all(
    modules.moduleNames.map(async (name, index) => {
      const modulePath = normalizePath9(
        path11.resolve(themeWebRoot, "components/modules/", name)
      );
...
function detectPackageManager(projectRoot) {
  if (fs3.existsSync(path11.join(projectRoot, "yarn.lock")))
    return "yarn";
  if (fs3.existsSync(path11.join(projectRoot, "pnpm-lock.yaml")))
    return "pnpm";
  return "npm";
}
...
var appServerRouteHandler = async (req, res, __next) => {
  try {
    ...
    const templatePath = path12.join(appWebRoot, "index.html");
    const serverEntryPath = isProd ? path12.join(appWebRoot, "server/serverEntry.js") : path12.join(appWebRoot, "serverEntry.tsx");
    let render;
    let template = await fs4.readFile(path12.join(templatePath), "utf-8");
    if (isProd) {
...
var createDevServerRoutes = async (viteServer, appLocals, storybookPort) => {
  ...
  if (isProd2) {
    const distAssetsPath = path13.join(appWebRoot, "assets");
...
async function getJsRenderingGates(portalId, env, personalAccessKey) {
  const {
    data: { enabledFeatures }
  } = await fetchAccessToken(personalAccessKey, env, portalId);
...
async function fetchWebsiteDomainNames(portalId) {
  ...
  const filteredDomains = domains.filter((domain) => {
    return domain.isUsedForBlogPost || domain.isUsedForSitePage || domain.isUsedForLandingPage;
  });
...
function checkFileExists(filePath) {
  return fs5.existsSync(filePath) && fs5.statSync(filePath).isFile();
}
function findHSProjectJson(startPath) {
  let currentDir = startPath;
  let dirLevel = 0;
  const maxDirLevel = 4;
  while (path14.dirname(currentDir) !== currentDir && dirLevel < maxDirLevel) {
    const filePath = path14.join(currentDir, HSPROJECT_JSON);
...
async function validateHSPlatform(projectConfigPath, portalId) {
  ...
  const hsProjectJSONContents = fs5.readFileSync(projectConfigPath, "utf8");
...
var isInModulesFolder = (filePath, projectRoot) => {
  ...
  const currentModuleId = path15.normalize(filePath);
  const moduleEntryPath = getEntryPathFromId(currentModuleId);
  const subFolderTokens = moduleEntryPath.split(path15.sep);
  return {
...
var fileContainsAutoGenComment = async (filePath) => {
  ...
  const firstLine = await new Promise(
    (resolve2) => reader.once("line", (line) => resolve2(line.trim()))
  );
  reader.close();
  stream2.close();
...
var getIndexFile = (filePath, projectRoot) => {
  ...
  return indexFileName ? { indexFilePath: path15.join(dir, indexFileName), isRootModuleFile } : void 0;
};
...
var maybeUpdateFields = async (filePath, viteServer, projectRoot) => {
  ...
  const indexFileName = path15.parse(indexFilePath).name;
  ...
  if (fields) {
    ...
    const typeFilePath = path15.join(path15.dirname(filePath), typeFileName);
    ...
    if (!hasAutoGenComment)
      return;
    const moduleFolderName = path15.basename(path15.dirname(filePath));
...
var startFieldsTypeGenWatcher = (viteServer, projectRoot) => {
  ...
  moduleSubpaths.forEach((modulePath) => {
    const resolvedPath = path15.resolve(projectRoot, modulePath);
...
async function createDevServer(webRoot, storybook, hubspotCliConfigPathOption, hubspotCliAccountNameOption, sslEnabledOption, fieldGenEnabledOption, themePreviewOptions, fieldBuilderEnabled) {
  ...
  const cmsAssetConfig = getCMSAssetConfig(webRoot);
  secretNamesFromConfig = getSecretNamesFromConfig(cmsAssetConfig);
  if (validateConfig()) {
    authedPortalId = getAccountId4(hubspotCliAccountNameOption);
    const env = getEnv3(hubspotCliAccountNameOption);
    if (!env) {
      throw new Error(
...
Local proxy functionality and module preview routes will not work without setting up authentication. You can work purely on modules locally at "/module/[module_name]", however the experience may be degraded due to missing account data.`
  ...
  const absoluteWebRoot = normalizePath10(path16.resolve(process.cwd(), webRoot));
  ...
  let _tokenStatus;
  if (themePreviewOptions && isUngatedForUnifiedThemePreview) {
  ...
  expressServer.locals.sslEnabled = sslEnabledOption;
  expressServer.use(bodyParser.json());
  const __dirname = path16.dirname(url2.fileURLToPath(new URL(import.meta.url)));
  let appWebRoot = normalizePath10(path16.resolve(__dirname, "../app"));
  if (true) {
    appWebRoot = path16.join(__dirname, "app");
  }
  ...
  if (!process.env.IN_CODESANDBOX && !process.env.CODESANDBOX_HOST) {
    startShadowDevServer(
      PORT,
      SHADOW_PORT,
      themePreviewOptions ? themePreviewOptions.dest : ""
    );
  }
  let extraServerInfo = "";
...
`;
  ...
  if (storybook) {
    const packageRoot = path16.dirname(url2.fileURLToPath(import.meta.url));
    const configPath = path16.resolve(packageRoot, "./storybook-main");
    sendTrackingRequest("start-storybook");
    let storybookBin;
    try {
      const sbPackagePath = await import.meta.resolve("storybook/package.json");
      const sbPackageDir = path16.dirname(url2.fileURLToPath(sbPackagePath));
      const packageJson = JSON.parse(
        fs7.readFileSync(path16.join(sbPackageDir, "package.json"), "utf-8")
      );
      storybookBin = path16.join(
        sbPackageDir,
        packageJson.bin.storybook || packageJson.bin
      );
    } catch (e) {
      console.error(
        wrapMessageInColor(
          "red",
          "Could not find storybook dependencies. The --storybook flag will be ignored."
        )
      );
      return;
    }
    ...
    const storybookProcess = spawn2(
      ...
      ],
      {
        ...
        env: {
          ...
          hsEnv: authedEnv,
          hublet: authedHublet,
          WEB_ROOT: path16.join(webRoot, "components")
        },
        stdio: "inherit"
      }
    );
    storybookProcess.on("error", (err) => {
      console.error(
        wrapMessageInColor(
          "red",
          "Failed to start storybook. The --storybook flag will be ignored."
        )
      );
      console.error(err);
    });
...
export {
  createDevServer_default as createDevServer,
  createViteDevServer_default as createViteDevServer
};
