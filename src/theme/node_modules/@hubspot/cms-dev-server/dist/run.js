...
import { getAccountConfig as getAccountConfig3 } from "@hubspot/local-dev-lib/config";
...
async function renderLocalProjectModulePreview(moduleName, authedPortalId, authedEnv, authedHublet, webRoot2, viteServer, jsRenderingGates, projectName, projectComponentPath, fullOriginalUrl, cookieValue, fieldOverrides = {}) {
  ...
  try {
    const authHeaders = {};
    const accountConfig = getAccountConfig3(authedPortalId);
    ...
    configureServer(server) {
      return () => {
        const getDomainInfo = (req) => {
          ...
          const env2 = authedEnv === "qa" ? "qa" : "";
          const previewDomain = `${authedPortalId}.hubspotpreview${env2}-${authedHublet}.com`;
          const protocol = sslEnabled ? "https" : "http";
...
import {
  ...
  getAccountConfig as getAccountConfig4,
  loadConfig,
  validateConfig
...
async function createDevServer(webRoot2, storybook2, hubspotCliConfigPathOption2, hubspotCliAccountNameOption2, sslEnabledOption2, fieldGenEnabledOption2, themePreviewOptions2, fieldBuilderEnabled2) {
  ...
  if (validateConfig()) {
    ...
    authedEnv = env2;
    personalAccessKey = getAccountConfig4(authedPortalId) && getAccountConfig4(authedPortalId).personalAccessKey;
...
`;
  ...
  if (storybook2) {
    ...
    try {
      ...
      const sbPackageDir = path17.dirname(url2.fileURLToPath(sbPackagePath));
      const packageJson = JSON.parse(
        fs8.readFileSync(path17.join(sbPackageDir, "package.json"), "utf-8")
      );
      storybookBin = path17.join(
        sbPackageDir,
