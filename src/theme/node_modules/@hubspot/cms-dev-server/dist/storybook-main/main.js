...
function getCMSAssetConfig(webRoot2) {
  const cmsAssetsPath = path.join(webRoot2, "cms-assets.json");
  const cmsThemePath = path.join(webRoot2, "theme.json");
...
function getCMSAssetConfigIfExists(webRoot2) {
  try {
    return getCMSAssetConfig(webRoot2);
  } catch (e) {
    console.error(
      `Error finding or parsing cms-assets.json in ${webRoot2}, but ignoring for now`
    );
    return {};
  }
}
...
var COMPONENT_EXTENSIONS, ENTRY_POINTS_PATH, MODULE_FOLDER_PATH, TEMPLATE_FOLDER_PATH, VIRTUAL_ISLAND_RUNTIME, VIRTUAL_CONFIG_SETUP, CSS_LANGS_RE, collectCssModules, collectCssInDev, badInlineJsChars, inlineJsEscapeChars, viteRoot, viteCommand, clientRenderIsland, virtualClientRenderIslandModuleId, scriptTagToLogServerMessages, getComponentExtension, hasComponentExtension, isStorybookStory, removeComponentExtension, COMBINED_REACT_BASE_PATH, ISLAND_RUNTIME_CONFIG_BASE_PATH, removeIndexFromPath, removeIndexOrTrailingJsFromPath, HEALTHCHECKS_ENABLED_DEFAULT, HEALTHCHECKS_STRICT_DEFAULT, BUILD_INFO_PATH, GATES, ENABLED_FEATURES_GATE_MAP, findAllComponentsIn;
var init_types_BxnmA6nH = __esm({
  "../cms-js-internal/dist/types-BxnmA6nH.js"() {
    COMPONENT_EXTENSIONS = [".jsx", ".tsx", ".js", ".ts"];
    ENTRY_POINTS_PATH = "components/partials/";
    MODULE_FOLDER_PATH = "components/modules/";
    TEMPLATE_FOLDER_PATH = "components/templates/";
    VIRTUAL_ISLAND_RUNTIME = "virtual:island-runtime";
    VIRTUAL_CONFIG_SETUP = "virtual:config-setup";
    CSS_LANGS_RE = /\.(css|less|sass|scss|styl|stylus|pcss|postcss|sss)(?:$|\?)/;
    collectCssModules = (modulePathToStartWith, viteServer, cssModulesImported = /* @__PURE__ */ new Set()) => {
    ...
    collectCssInDev = (modulePathToStartWith, viteServer, cssModulesImported = /* @__PURE__ */ new Set()) => {
...
`;
    ...
    getComponentExtension = (componentPath) => {
    ...
    COMBINED_REACT_BASE_PATH = "https://static.hsappstatic.net/cms-js-static/ex/js/react/v18";
    ISLAND_RUNTIME_CONFIG_BASE_PATH = "https://static.hsappstatic.net/cms-js-static/ex/js/island-runtime/v1";
    removeIndexFromPath = (id) => {
      return id.replace(/\/index\.[jt]sx?$/, "");
    };
    removeIndexOrTrailingJsFromPath = (id) => {
      return removeIndexFromPath(id).replace(/\.[jt]sx?$/, "");
    };
    HEALTHCHECKS_ENABLED_DEFAULT = true;
    HEALTHCHECKS_STRICT_DEFAULT = false;
    ...
    GATES = [
      ...
      "CMS:JSRenderer:CompressResponse"
    ];
    ...
    findAllComponentsIn = (webRoot2, componentsFolder) => {
      const fullComponentsFolderPath = join(webRoot2, componentsFolder);
...
__export(dist_exports, {
  ...
  DndAreaSlot: () => DndAreaSlot,
  ENABLED_FEATURES_GATE_MAP: () => ENABLED_FEATURES_GATE_MAP,
  ENTRY_POINTS_PATH: () => ENTRY_POINTS_PATH,
  GATES: () => GATES,
  GlobalPartialSlot: () => GlobalPartialSlot,
  MODULE_FOLDER_PATH: () => MODULE_FOLDER_PATH,
  ModuleSlot: () => ModuleSlot,
  TEMPLATE_FOLDER_PATH: () => TEMPLATE_FOLDER_PATH,
  VIRTUAL_CONFIG_SETUP: () => VIRTUAL_CONFIG_SETUP,
  VIRTUAL_ISLAND_RUNTIME: () => VIRTUAL_ISLAND_RUNTIME,
  areBuildHealthChecksEnabled: () => areBuildHealthChecksEnabled,
  clientIslandEntryPlugin: () => clientIslandEntryPlugin,
  collectCssInDev: () => collectCssInDev,
  collectCssModules: () => collectCssModules,
  createIslandsGlobalJsForClientInDev: () => createIslandsGlobalJsForClientInDev,
  doHealthChecksFailTheBuild: () => doHealthChecksFailTheBuild,
  escapeForInlineJS: () => escapeForInlineJS,
  findAllComponentsIn: () => findAllComponentsIn,
  findAllModulesInRoot: () => findAllModulesInRoot,
  findAllPartialsInRoot: () => findAllPartialsInRoot,
  findCmsModules: () => findCmsModules,
  getBaseAssetUrlFromBuild: () => getBaseAssetUrlFromBuild,
  getBuildInfo: () => getBuildInfo,
  getCMSAssetConfig: () => getCMSAssetConfig,
  getCMSAssetConfigIfExists: () => getCMSAssetConfigIfExists,
  getCombinedReactUrl: () => getCombinedReactUrl,
  getComponentExtension: () => getComponentExtension,
  getEntryPointHash: () => getEntryPointHash,
  getEntryPointHashWithIndexSuffix: () => getEntryPointHashWithIndexSuffix,
  getIslandRuntimeConfigUrl: () => getIslandRuntimeConfigUrl,
  getIslandUrls: () => getIslandUrls,
  getServerGlobalsToInject: () => getServerGlobalsToInject,
  getServerIncludePrebundleDepPatterns: () => getServerIncludePrebundleDepPatterns,
  hasComponentExtension: () => hasComponentExtension,
  includesHublDataReference: () => includesHublDataReference,
  isStorybookStory: () => isStorybookStory,
  isStylesheetAsset: () => isStylesheetAsset,
  isUngatedFor: () => isUngatedFor,
  jsonStringifyAndEscapeForInlineJS: () => jsonStringifyAndEscapeForInlineJS,
  namespaceModulePropsShape: () => namespaceModulePropsShape,
  noncePlaceholderPlugin: () => noncePlaceholderPlugin,
  removeComponentExtension: () => removeComponentExtension,
  removeIndexFromPath: () => removeIndexFromPath,
  removeIndexOrTrailingJsFromPath: () => removeIndexOrTrailingJsFromPath,
  renderComponentOnServer: () => renderComponentOnServer,
  renderFieldsToJson: () => renderFieldsToJson,
  renderOnServer: () => renderOnServer,
  renderStreamToPromise: () => renderStreamToPromise,
  resolveIslandPlugin: () => resolveIslandPlugin,
  resolveIslandToPlainModuleNoopPlugin: () => resolveIslandToPlainModuleNoopPlugin,
  resolveIslandToPlainModulePlugin: () => resolveIslandToPlainModulePlugin,
  scriptTagToLogServerMessages: () => scriptTagToLogServerMessages,
  serializeIslandDataForInlineScript: () => serializeIslandDataForInlineScript,
  setIn: () => setIn,
  ungatedAtBuildTimeFor: () => ungatedAtBuildTimeFor,
  virtualClientRenderIslandModuleId: () => virtualClientRenderIslandModuleId
});
...
function toRelativePath(config, path3) {
  return path3.replace(`${config.root}/`, "");
}
...
function analyzeSourceCodeUsages(ctx, moduleInfo, config, code) {
  ...
  traverse(sourceAst, {
    ImportDeclaration(path3) {
      if (t.isStringLiteral(path3.node.source, {
    ...
    JSXOpeningElement(path3) {
      if (t.isJSXIdentifier(path3.node.name, {
        name: moduleComponentLocalImportName
      })) {
        let cmsModuleName;
        let cmsModulePath;
        let cmsModuleLabel;
        for (const prop of path3.node.attributes) {
...
function GlobalPartialSlot(props) {
  return /* @__PURE__ */ jsx(
    "div",
    {
      suppressHydrationWarning: true,
      dangerouslySetInnerHTML: {
        __html: `<!-- hs-global-partial-placeholder-${props.name} -->`
      }
    }
  );
}
var setProperty, getProperty, hasProperty, serializeIslandDataForInlineScript, writeSharedStateToJsGlobal, createIslandsGlobalJsForClientInDev, traverse, CMS_MODULE_COMPONENT_NAME, DND_AREA_COMPONENT_NAME, PLUGIN_NAME, promiseForNewWritableStream, renderStreamToPromise, createServerRenderContextV22, createServerRenderContext2, renderServerSidePropsWithAsyncLocalStoreContext, bootStrapRenderContext;
var init_dist = __esm({
  "../cms-js-internal/dist/index.js"() {
    init_types_BxnmA6nH();
    init_types_BxnmA6nH();
    ({ setProperty, getProperty, hasProperty } = dotProp);
    serializeIslandDataForInlineScript = (tweakedIslandsArray, perServerRequestData, baseIndent = 0, prettyPrint, isUngatedForClientOnlySupplementalFieldSerialization = false) => {
    ...
    writeSharedStateToJsGlobal = (islands, perServerRequestData, indent = 0, prettyPrint) => {
...
${indentString}`);
    ...
    createIslandsGlobalJsForClientInDev = (roots, perServerRequestData, viteDevServer, indent, prettyPrint, isUngatedForClientOnlySupplementalFieldSerialization = false) => {
    ...
    CMS_MODULE_COMPONENT_NAME = "Module";
    DND_AREA_COMPONENT_NAME = "DndArea";
    PLUGIN_NAME = "findCmsModules";
    ...
    renderStreamToPromise = async (reactElements, options) => {
...
var main_default = {
  ...
  previewHead: (head) => {
    ...
    <script type="module">
      ...
      initConfigSingletonFromJSON(${jsonStringifyAndEscapeForInlineJS(
      {
        portalId: process.env.portalId,
        hublet: process.env.hublet,
        hsEnv: process.env.hsEnv,
        hsDeployed: false,
        hsJSRGates: jsRenderingGates
      },
      { prettyPrint: false }
    )})
    </script>
  ...
  viteFinal: async (config) => {
    const { resolveIslandPlugin: resolveIslandPlugin2 } = await Promise.resolve().then(() => (init_dist(), dist_exports));
    const viteConfig = {
      define: {
        global: "globalThis",
        "process.env": "{}",
      ...
      optimizeDeps: {
        include: ["react", "react-dom", "react-dom/client", "buffer"],
        exclude: ["fsevents"]
      },
      css: {
        postcss: webRoot
      ...
      plugins: [
        ...
        {
          name: "provide-nodejs-globals",
          config(__config, { command }) {
            if (command === "serve") {
              config.define = config.define || {};
              config.define.global = "globalThis";
              config.define.Buffer = "Buffer";
            }
          },
          transformIndexHtml: {
            order: "pre",
    ...
    const configNoReact = {
      ...config,
      // replacing the default usage of `@vite/plugin-react` (it emits multiple
      // sub-plugins listed here) to add our babel plugin configuration
    ...
    return mergeConfig(configNoReact, viteConfig);
  }
};
export {
